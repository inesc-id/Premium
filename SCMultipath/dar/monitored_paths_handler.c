...
//	x->last_time_info = (char*) malloc(sizeof(char)*TIME_MAX_LENGHT);
	x->previous_time_info = NULL;
	x->last_time_info = NULL;
...
void save_unstructured_path(char *source_ip, char *relay_ip, char *destination_ip)
{
	path_status path = new_path_status(source_ip, relay_ip, destination_ip);
	save_path_status(path);	
...
void show_path_status_simple(path_status path)
{
	printf("[%s] %s -> %s -> %s\n", 
...
void show_path_status_structured(path_status path) 
{
	printf("-----------> Path Status <---------- \n");
...
/*
	...
	if (foundPathNode == NULL)
	{
		...
		printf("Found Path: \n");
		show_path_status_complex(foundPathNode->item);
	}
*/
}
...
void show_hijacked_paths(int was_hijacked)
{
	...
	while(path_node != NULL){
		show_path_status_structured(path_node->item);
...
void show_all_nodes_p(path_node_link node)
{
	...
	show_path_status_structured(node->item);
